namespace WebApi {
    ...
    public class BalanceHidricoController : ApiController {
        ...
        public IHttpActionResult GetDatosHidricosList(int? idRegante, string idUnidadCultivo, int? idMunicipio, string idCultivo, string fecha) {
            try {
                ...
                if (isAdmin == false && DB.LaUnidadDeCultivoPerteneceAlReganteEnLaTemporada(idUnidadCultivo, (int)idRegante, idTemporada) == false)
                    return Unauthorized();
                object lDatosHidricos = DB.DatosHidricosList(idRegante, idUnidadCultivo, idMunicipio, idCultivo, DateTime.Parse(fecha), isAdmin);
        ...
        [HttpGet]
        [Route("api/ResumenDiario/{idUnidadCultivo}/{IdTemporada}")]
        public IHttpActionResult ResumenDiario(string idUnidadCultivo, string idTemporada) {
            try {
                //payaso
                /*
                ClaimsIdentity identity = Thread.CurrentPrincipal.Identity as ClaimsIdentity;
